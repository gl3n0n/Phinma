...
<asp:SqlDataSource ID="dsVendorAuthenticated" runat="server" ConnectionString="<%$ ConnectionStrings:AVAConnectionString %>"
            SelectCommand="SELECT t1.VendorId, t1.CompanyName, t1.AuthenticationTicket, t1.DateCreated, t1.approvedbyDnbDate FROM tblVendor t1 WHERE t1.IsAuthenticated = 1 AND t1.Status > 1 AND t1.Status <> 9 AND t1.EndorsedTo=@EndorsedTo ORDER BY t1.approvedbyDnbDate DESC" >
    <SelectParameters>
        <asp:SessionParameter Name="EndorsedTo" Type="Int32" SessionField="UserId" />
    </SelectParameters>
	    </asp:SqlDataSource>
<asp:GridView ID="GridView1" runat="server" DataSourceID="dsVendorAuthenticated" 
    ...
    <Columns>
        <asp:TemplateField HeaderText="Company Name" InsertVisible="False" SortExpression="CompanyName" ItemStyle-HorizontalAlign="Center">
            ...
            <ItemTemplate>
                &nbsp;<%--<asp:LinkButton ID="lnkRefNo" runat="server" Text='<%# Bind("CompanyName") %>' CommandArgument='<%# Bind("VendorId") %>' CommandName="Details"></asp:LinkButton>--%><a href="<%# "vendor_Home.aspx?VendorId=" + Eval("VendorId") %>" class="iframe" title='<%# Eval("CompanyName") %>' ><%# Eval("CompanyName") %></a><%--&nbsp;<asp:Label ID="Label0" runat="server" Text='<%# Bind("CompanyName") %>'></asp:Label>--%>
            </ItemTemplate>
        </asp:TemplateField>
