...
public partial class vmofficer_VendorForNotification_List : System.Web.UI.Page
{
            ...
            using (cmd = new SqlCommand(query, conn))
            {
                ...
                if (oReader.HasRows)
                {
                    while (oReader.Read())
                    {
                        ...
                        string VendorEmail = oReader["conSalEmail"].ToString();
                        ...
                        query = "UPDATE tblVendors SET VendorCode=@VendorCode, Vendor_Code=@VendorCode, VendorEmail=@VendorEmail WHERE VendorId=@VendorId";
    ...
    private string CreateNotificationBody(string cfromName, string ctoName, string cAuthenticationNumber, string cVendorName, string VendorIdx, string Username, string Password)
    {
        ...
        sb.Append("<a href='http://10.10.20.61/login.aspx'>http://10.10.20.61/login.aspx</a><br><br>");
        sb.Append(sTxt);
        sb.Append("</p>");
