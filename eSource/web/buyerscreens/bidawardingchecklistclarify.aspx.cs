...
public partial class web_buyerscreens_bidawardingchecklistclarify : System.Web.UI.Page
{
    ...
    void SaveToDB()
    {
        ...
        sCommand = sCommand + " SS_NoBidders=" + ss_nobidders + ", SS_NoBidsRcvd=" + ss_nobidsrcvd + ", SS_TechCompliance=" + ss_techcompliance + ", Status=" + status1 + ",  SavingsGen_Amount=" + lblNegDisc + ", SavingsGen_PctSpend=" + lblPercentSpend + ", descPaymentTerms='" + Request.Form["descPaymentTerms"].ToString().Replace("'", "''") + "', ss_crc_no='" + Request.Form["ss_crcnumber"].Replace("'", "''") + "'  WHERE BidRefNo =" + Session["BuyerBidForBac"];
    ...
    {
        string connstring = HttpContext.Current.Session["ConnectionString"].ToString();
        string sCommand, sLastCommented;
        SqlDataReader oReader;
        ...
        sCommand = sCommand + Session["BuyerBidForBac"].ToString() + ", " + Session["UserId"] + ", '" + Request.Form["txtClarify"].Replace("'", "''").Replace("\n", "<br />") + "', GETDATE())";
        SqlHelper.ExecuteNonQuery(connstring, CommandType.Text, sCommand);
        ...
        if (sLastCommented == "ApprovedDt_00") //this buyer
        {
            sCommand = "UPDATE tblBacBidItems SET Status=1 WHERE BacRefNo = " + Session["BuyerBacRefNo"];
        }
        else
        {
            sCommand = "UPDATE tblBacBidItems SET " + sLastCommented + "= GETDATE(), Status=1 WHERE BacRefNo=" + Session["BuyerBacRefNo"];
        }
        SqlHelper.ExecuteNonQuery(connstring, CommandType.Text, sCommand);
    ...
    protected void UpdateCommercialDetails()
    {
        ...
        foreach (GridViewRow oGrid in gvItems.Rows)
        {
            ...
            LinkButton oTxt2 = ((LinkButton)oGrid.FindControl("lblAddItem"));
            sCommand = "UPDATE tblBACEvaluationDetails SET PR_No='" + oPRNo.Text.Replace("'", "") + "' , ItemName='" + oItemD.Text.Replace("'", "") + "' WHERE BidRefNo=" + Session["BuyerBidForBac"] + " ";
        ...
        foreach (DataListItem oItem in lstCommDetails.Items)
        {
            ...
            foreach (RepeaterItem oRptItem in oRpt.Items)
            {
                if (oRptItem.ItemType == ListItemType.Item || oRptItem.ItemType == ListItemType.AlternatingItem)
                {
                    ...
                    sCommand = "UPDATE tblBACEvaluationDetails SET Qty=" + oTxt.Text + " WHERE BidRefNo=" + Session["BuyerBidForBac"] + " ";
                    sCommand = sCommand + "AND BidDetailNo=" + sBidDetailNo1 + " AND BidTenderNo='" + sBidTender1 + "' AND UserId=" + Session["UserId"];
                    SqlHelper.ExecuteNonQuery(connstring, CommandType.Text, sCommand);
                }
            }
            ...
            foreach (RepeaterItem oRptItem in oRpt.Items)
            {
                if (oRptItem.ItemType == ListItemType.Item || oRptItem.ItemType == ListItemType.AlternatingItem)
                {
                    ...
                    sCommand = "UPDATE tblBACEvaluationDetails SET Currenzy='" + oTxt.Text.Replace("'", "''").Trim() + "' WHERE BidRefNo=" + Session["BuyerBidForBac"] + " ";
                    sCommand = sCommand + "AND BidDetailNo=" + sBidDetailNo1 + " AND BidTenderNo='" + sBidTender1 + "' AND UserId=" + Session["UserId"];
                    SqlHelper.ExecuteNonQuery(connstring, CommandType.Text, sCommand);
                }
            }
    ...
    private bool SendEmailNotification()
    {
        ...
        //GET CLARIFIEDBY EMAIL AS THE RECEPIENT
        sCommand = "select PurchasingID, Name1, EmailAdd FROM ( ";
        sCommand = sCommand + "SELECT t1.PurchasingID, t2.FirstName + ' ' + t2.MiddleName + ' ' + t2.LastName AS Name1, t2.EmailAdd FROM tblSupervisor t1 ";
        sCommand = sCommand + "INNER JOIN tblPurchasing t2 ON t1.PurchasingID=t2.PurchasingID WHERE BuyerID=" + Session["UserId"] + " ";
        sCommand = sCommand + "UNION ";
        sCommand = sCommand + "SELECT t1.BacID PurchasingID, t1.FirstName + ', ' + t1.MiddleName + ' ' + t1.LastName AS Name1, t1.EmailAdd FROM tblBidAwardingCommittee t1 ) as a1 INNER JOIN (SELECT TOP 1 * FROM tblBACClarifications d  ";
        sCommand = sCommand + "WHERE d.ToUserId = " + Session["UserId"] + "  ANd d.BidRefNo = " + Session["BuyerBidForBac"] + " ORDER BY d.DatePosted desc) t2 ON t2.FrUserId =a1.PurchasingID ";
