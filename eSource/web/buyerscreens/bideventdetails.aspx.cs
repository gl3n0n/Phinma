    ...
    protected void Page_Load(object sender, System.EventArgs e)
    {
        ...
        if (Session[Constant.SESSION_LASTPAGE] != null)
        {
            if (Session[Constant.SESSION_LASTPAGE].ToString() == "~/web/buyerscreens/bidsforeval.aspx" 
                || Session[Constant.SESSION_LASTPAGE].ToString() == "~/web/buyerscreens/endorsedbidtenders.aspx"
                || Session[Constant.SESSION_LASTPAGE].ToString() == "~/web/buyerscreens/awardedbiditems.aspx"
                )
            {
        ...
        if (!IsPostBack)
        {
            ...
            lnkViewReport.Attributes.Add("onclick", "window.open('../reports/eventtenderscanvass.aspx?brn=" + Session[Constant.SESSION_BIDREFNO].ToString().Trim() + "','r1', 'toolbar=no, menubar=no, width=800; height=600, top=80, left=80, resizable=yes , scrollbars=yes');");
