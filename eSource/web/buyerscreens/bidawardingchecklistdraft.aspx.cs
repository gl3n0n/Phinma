...
public partial class web_buyerscreens_bidawardingchecklistdraft : System.Web.UI.Page
{
    ...
    void SaveToDB()
    {
        ...
        sCommand = sCommand + "BidRefNo = " + Session["BuyerBidForBac"] + ", Budgeted=" + Request.Form["Budgeted"] + ", CompanyID=" + Request.Form["CompanyId"] + ", ItemDesc='" + sItemDesc.Replace("'", "''") + "', BuyerID=" + Session["UserId"] + ", SAPPRNo='" + iSAP + "', BidAmount=" + cAmt + ", SAPPRDate='" + dPRdate + "', RcvdDate= '" + dReceived + "', ";
        sCommand = sCommand + " SS_NoBidders=" + ss_nobidders + ", SS_NoBidsRcvd=" + ss_nobidsrcvd + ", SS_TechCompliance=" + ss_techcompliance + ", Status=" + status1 + ",  SavingsGen_Amount=" + lblNegDisc  + ", SavingsGen_PctSpend=" + lblPercentSpend + ", descPaymentTerms='" + Request.Form["descPaymentTerms"].Replace("'", "''") + "', ss_crc_no='" + Request.Form["ss_crcnumber"].Replace("'", "''") + "'  WHERE BidRefNo =" + Session["BuyerBidForBac"];
    ...
    protected void doBuyerBidForBac()
    {
        ...
        if (oReader.HasRows)
        {
            ...
            lblAccCost.Text = String.Format("{0:0,0.00}", Convert.ToDouble(oReader["AccumulativeCost"].ToString()));
            lblNegDisc.Text = String.Format("{0:0,0.00}", Convert.ToDouble(oReader["SavingsGen_Amount"].ToString()));
            lblPercentSpend.Text = String.Format("{0:0,0.00}", Convert.ToDouble(oReader["SavingsGen_PctSpend"].ToString()));
        ...
        iApproverCount = 0;
        dsApprover.SelectCommand = "IF EXISTS (SELECT 1 FROM tblBACSupportingDocuments WHERE BidRefNo=" + Session["BuyerBidForBac"] + " AND DocuName='bfa_LowestPr') BEGIN SELECT t1.BACId, t1.Initials AS Name1, t1.LimitLowestChar AS ApprovingMaxLimit, t1.LimitLowestTo, t1.LimitNonLowestTo, t1.Committee, CASE WHEN t2.Approver IS NULL THEN 0 ELSE 1 END AS isApprover FROM vwApprovers t1 LEFT OUTER JOIN vmBacBidApprovers t2 on t1.BACId = t2.Approver AND BidRefNo =" + Session["BuyerBidForBac"] + "  WHERE t1.Approver = 0 AND t1.OIC = 0 AND t1.LimitLowest <= (SELECT SUM(TotalCost) FROM tblBACEvaluationDetails WHERE BidRefNo=" + Session["BuyerBidForBac"] + " AND Chkd=1) UNION SELECT t1.BACId, t1.Initials AS Name1, t1.LimitLowestChar AS ApprovingMaxLimit, t1.LimitLowestTo, t1.LimitNonLowestTo, t1.Committee, CASE WHEN t2.Approver IS NULL THEN 0 ELSE 1 END AS isApprover FROM vwApprovers t1 LEFT OUTER JOIN vmBacBidApprovers t2 on t1.BACId = t2.Approver AND BidRefNo =" + Session["BuyerBidForBac"] + "  WHERE t1.Approver = 1 AND t1.OIC = 0 AND (SELECT SUM(TotalCost) FROM tblBACEvaluationDetails WHERE BidRefNo=" + Session["BuyerBidForBac"] + " AND Chkd=1) BETWEEN t1.LimitLowest AND t1.LimitLowestTo ORDER BY LimitLowestTo ASC, Committee, Name1 END ELSE BEGIN SELECT t1.BACId, t1.Initials AS Name1, t1.LimitNonLowestChar AS ApprovingMaxLimit, t1.LimitLowestTo, t1.LimitNonLowestTo, t1.Committee, CASE WHEN t2.Approver IS NULL THEN 0 ELSE 1 END AS isApprover FROM vwApprovers t1 LEFT OUTER JOIN vmBacBidApprovers t2 on t1.BACId = t2.Approver AND BidRefNo =" + Session["BuyerBidForBac"] + " WHERE t1.Approver = 0 AND t1.OIC = 0 AND t1.LimitNonLowest <= (SELECT SUM(TotalCost) FROM tblBACEvaluationDetails WHERE BidRefNo=" + Session["BuyerBidForBac"] + " AND Chkd=1) UNION SELECT t1.BACId, t1.Initials AS Name1, t1.LimitNonLowestChar AS ApprovingMaxLimit, t1.LimitLowestTo, t1.LimitNonLowestTo, t1.Committee, CASE WHEN t2.Approver IS NULL THEN 0 ELSE 1 END AS isApprover FROM vwApprovers t1 LEFT OUTER JOIN vmBacBidApprovers t2 on t1.BACId = t2.Approver AND BidRefNo =" + Session["BuyerBidForBac"] + " WHERE t1.Approver = 1 AND t1.OIC = 0 AND (SELECT SUM(TotalCost) FROM tblBACEvaluationDetails WHERE BidRefNo=" + Session["BuyerBidForBac"] + " AND Chkd=1) BETWEEN t1.LimitNonLowest AND t1.LimitNonLowestTo ORDER BY LimitNonLowestTo ASC, LimitLowestTo ASC, Committee, Name1 END";
        dsApprover.DataBind();
        ...
        foreach (DataListItem oItem in lstCommDetails.Items)
        {
            ...
            foreach (RepeaterItem oRptItem in oRpt.Items)
            {
                if (oRptItem.ItemType == ListItemType.Item || oRptItem.ItemType == ListItemType.AlternatingItem)
                {
                    ...
                    sCommand = "UPDATE tblBACEvaluationDetails SET VendorName='" + oTxt2.Text.Trim().Replace("'", "''").Trim() + "' WHERE BidRefNo=" + Session["BuyerBidForBac"] + " ";
            ...
            foreach (RepeaterItem oRptItem in oRpt.Items)
            {
                if (oRptItem.ItemType == ListItemType.Item || oRptItem.ItemType == ListItemType.AlternatingItem)
                {
                    ...
                    sCommand = "UPDATE tblBACEvaluationDetails SET Qty=" + oTxt.Text.Trim() + " WHERE BidRefNo=" + Session["BuyerBidForBac"] + " ";
                    sCommand = sCommand + "AND BidDetailNo=" + sBidDetailNo1 + " AND BidTenderNo='" + sBidTender1 + "' AND UserId=" + Session["UserId"];
                    SqlHelper.ExecuteNonQuery(connstring, CommandType.Text, sCommand);
                }
            }
            ...
            foreach (RepeaterItem oRptItem in oRpt.Items)
            {
                if (oRptItem.ItemType == ListItemType.Item || oRptItem.ItemType == ListItemType.AlternatingItem)
                {
                    ...
                    sCommand = "UPDATE tblBACEvaluationDetails SET UnitMeasure='" + oTxt.Text.Trim().Replace("'", "''").Trim() + "' WHERE BidRefNo=" + Session["BuyerBidForBac"] + " ";
                    sCommand = sCommand + "AND BidDetailNo=" + sBidDetailNo1 + " AND BidTenderNo='" + sBidTender1 + "' AND UserId=" + Session["UserId"];
                    SqlHelper.ExecuteNonQuery(connstring, CommandType.Text, sCommand);
                }
            }
            ...
            foreach (RepeaterItem oRptItem in oRpt.Items)
            {
                if (oRptItem.ItemType == ListItemType.Item || oRptItem.ItemType == ListItemType.AlternatingItem)
                {
                    ...
                    sCommand = "UPDATE tblBACEvaluationDetails SET Currenzy='" + oTxt.Text.Trim().Replace("'", "''").Trim() + "' WHERE BidRefNo=" + Session["BuyerBidForBac"] + " ";
                    sCommand = sCommand + "AND BidDetailNo=" + sBidDetailNo1 + " AND BidTenderNo='" + sBidTender1 + "' AND UserId=" + Session["UserId"];
                    SqlHelper.ExecuteNonQuery(connstring, CommandType.Text, sCommand);
                }
            }
            ...
            foreach (RepeaterItem oRptItem in oRpt.Items)
            {
                if (oRptItem.ItemType == ListItemType.Item || oRptItem.ItemType == ListItemType.AlternatingItem)
                {
                    ...
                    sCommand = "UPDATE tblBACEvaluationDetails SET UnitCost=" + oTxt.Text.Replace(",", "") + " WHERE BidRefNo=" + Session["BuyerBidForBac"] + " ";
                    sCommand = sCommand + "AND BidDetailNo=" + sBidDetailNo1 + " AND BidTenderNo='" + sBidTender1 + "' AND UserId=" + Session["UserId"];
                    SqlHelper.ExecuteNonQuery(connstring, CommandType.Text, sCommand);
                }
            }
            ...
            foreach (RepeaterItem oRptItem in oRpt.Items)
            {
                if (oRptItem.ItemType == ListItemType.Item || oRptItem.ItemType == ListItemType.AlternatingItem)
                {
                    ...
                    sCommand = "UPDATE tblBACEvaluationDetails SET TotalCost=" + oTxt.Text.Replace(",", "") + " WHERE BidRefNo=" + Session["BuyerBidForBac"] + " ";
                    sCommand = sCommand + "AND BidDetailNo=" + sBidDetailNo1 + " AND BidTenderNo='" + sBidTender1 + "' AND UserId=" + Session["UserId"];
                    SqlHelper.ExecuteNonQuery(connstring, CommandType.Text, sCommand);
                }
            }
            ...
            sCommand = "UPDATE tblBacBidItems SET AccumulativeCost = ISNULL((select sum(TotalCost) from tblBACEvaluationDetails WHERE BidRefNo=" + Session["BuyerBidForBac"] + " ";
